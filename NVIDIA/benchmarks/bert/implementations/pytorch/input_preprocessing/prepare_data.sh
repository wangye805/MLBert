...
function usage()
{
   ...
   optional arguments:
     ...
     -c, --packed-data     run genration of packed data as the final step (needs multi cpu server with large memory)
...
[ ${PACKED_DATA} -eq 0 ] && exit 0 

#################################
### PACKED DATA PREPROCESSING ###
#################################

${SCRIPT_DIR}/packed_data/prepare_packed_data.sh --skip-download --outputdir ${DATADIR} --shards ${SHARDS}
